<div id="main-content" className="activiti-list">
    <div className="page-title en">
        <div className="container">
            <div className="head-title">
                <div>
                    <div className="share-page en">
                        <h2>Share this page</h2>
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.facebook.com/sharer/sharer.php?u=http://51.136.51.121/en/activities&amp;src=sdkpreparse"
                            className="fb-xfbml-parse-ignore"
                        >
                            <span>acebook</span>
                            <i aria-label="Facebook" className="fa fa-facebook">
                                &nbsp;
                            </i>
                        </a>
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="http://twitter.com/share?text=Studio 5   Activities&amp;url=http://51.136.51.121/en/activities"
                        >
                            <span>acebook</span>
                            <i aria-label="Twitter" className="fa-brands fa-x-twitter">
                                &nbsp;
                            </i>
                        </a>
                    </div>
                </div>
                <h2
                    id="pageHeading"
                    role="heading"
                    tabIndex={0}
                    aria-label="page heading Activities"
                >
                    Activities
                </h2>
            </div>
        </div>
    </div>


    <div className="container">
        <nav className="breadcrumb" id="breadcrumb-wrap" aria-label="breadcrumb">
            <ul>
                <li className="breadcrumb-item">
                    <a href="/en/">Home</a>{" "}
                </li>
                <li className="breadcrumb-item">
                    <a tabIndex={0} aria-current="page" href="/en/activities">
                        <span>Activities</span>
                    </a>
                </li>
            </ul>
        </nav>
    </div>
    <span className="multi-square one">
        <b>
            <i />
        </b>
    </span>


    <div id="skipContent">
        <div id="main-container">
            <div className="container">
                <form method="post" name="search" className="search">
                    <div className="row topFormHolder">
                        <div className="col s12 m6 l3">
                            <div className="input-field item">
                                <input
                                    name="Title"
                                    placeholder="Title"
                                    id="title"
                                    type="text"
                                    aria-label="Activity search title"
                                    defaultValue=""
                                />
                                <label htmlFor="title" style={{ left: 0, right: "auto" }}>
                                    Title
                                </label>
                                <span className="helper-text" />
                            </div>
                        </div>
                        <div className="col s12 m6 l3 date-popup">
                            <div className="input-field mobileSelect item en">
                                <label
                                    htmlFor="activityType"
                                    className="active"
                                    style={{ left: 0, right: "auto" }}
                                >
                                    Activity type
                                </label>
                                <select
                                    className="browser-default"
                                    id="activityType"
                                    tabIndex={0}
                                >
                                    <option value="" disabled="">
                                        Select activity type
                                    </option>
                                    <option value="All">All</option>
                                    <option value={77}>Public activity</option>
                                    <option value={78}>School activity</option>
                                </select>
                                <span className="helper-text" />
                            </div>
                        </div>
                        <div className="col s12 m6 l3 date-popup">
                            <div className="input-field item">
                                <div className="react-datepicker-wrapper">
                                    <div className="react-datepicker__input-container">
                                        <input
                                            name="date"
                                            autoComplete="off"
                                            id="date"
                                            placeholder="Date"
                                            type="date"
                                            className="hiddenDob customDateField"
                                            defaultValue=""
                                        />
                                        <button
                                            aria-label="choose date button"
                                            type="button"
                                            className="example-custom-input"
                                        >
                                            <span aria-hidden="true" className="material-icons">
                                                date_range
                                            </span>
                                        </button>
                                    </div>
                                </div>
                                <label htmlFor="date" style={{ left: 0, right: "auto" }}>
                                    Date
                                </label>
                                <span className="helper-text" />
                            </div>
                        </div>
                        <div className="col s12 m6 l3 valign-wrapper center-align search-btn-holder">
                            <button
                                type="submit"
                                className="btn noWidth profileBtn yellow waves-effect waves-light"
                                aria-label="Activity search"
                            >
                                Search <i className="material-icons en">arrow_forward</i>
                            </button>
                            <button
                                type="button"
                                aria-label="Activity search clear"
                                className="btn noWidth blue-btn waves-effect waves-light"
                            >
                                Clear
                            </button>
                        </div>
                    </div>
                </form>


                <div className="row">
                    <div className="col s7 result-status" />
                    <div className="col s5 result-type">
                        <div className="input-field col s12 mobileSelect filter-select en">
                            <label htmlFor="result-type">Select Activity</label>
                            <select id="result-type" className="browser-default" tabIndex={0}>
                                <option value="All">All</option>
                                <option value="competition">Competitions</option>
                                <option value="session">Workshops</option>
                                <option value="event">Events</option>
                                <option value="challenge">Challenges</option>
                                <option value="project">Projects</option>
                            </select>
                        </div>
                    </div>
                </div>
                <ul className="activities-list" aria-label="activities">
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2024-01/Image_20240111_094103_391.png"
                                alt="Hackathon- Passive Cooking image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Creativity Zone">
                                <a
                                    aria-label="Hackathon- Passive Cooking Activity mixed 19-24 Years Old Start date 26 Jan, 2024 03:00 pm and End date 27 Jan, 2024 07:00 pm"
                                    href="/en/activity/57640"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        Hackathon- Passive Cooking
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>19-24 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    The Ministry of Communications and Information Technology
                                    (MCIT) is excited to invite you to an...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            26 Jan, 2024 <br />
                                            <small>03:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            27 Jan, 2024
                                            <br />
                                            <small>07:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2024-01/Image_20240111_094103_480.png"
                                alt="BIO Matters image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Technology Zone">
                                <a
                                    aria-label="BIO Matters Activity mixed 19-24 Years Old Start date 20 Jan, 2024 03:00 pm and End date 20 Jan, 2024 05:00 pm"
                                    href="/en/activity/57638"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        BIO Matters
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>19-24 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash Creativity with Sustainable Design! &nbsp; ?
                                    Experiment, Innovate, Transform! Join us at...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            20 Jan, 2024 <br />
                                            <small>03:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            20 Jan, 2024
                                            <br />
                                            <small>05:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2024-01/Image_20240111_094103_504.png"
                                alt="Recipe with AI image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Software Zone">
                                <a
                                    aria-label="Recipe with AI Activity mixed 15-18 Years Old Start date 19 Jan, 2024 03:00 pm and End date 19 Jan, 2024 05:00 pm"
                                    href="/en/activity/57639"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        Recipe with AI
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>15-18 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash the Future of Recipes Innovation! &nbsp; ? Code,
                                    Create, Cook! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            19 Jan, 2024 <br />
                                            <small>03:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            19 Jan, 2024
                                            <br />
                                            <small>05:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231225_124703_437_0.png"
                                alt="3D Molds image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Investigation Zone">
                                <a
                                    aria-label="3D Molds Activity mixed 11-14 Years Old Start date 13 Jan, 2024 02:00 pm and End date 13 Jan, 2024 04:00 pm"
                                    href="/en/activity/57511"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        3D Molds
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>11-14 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash Your Creativity in 3D Confectionery! &nbsp; Model,
                                    Mold, Decorate! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            13 Jan, 2024 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            13 Jan, 2024
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231225_124703_515_0.png"
                                alt="Solar Oven image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Creativity Zone">
                                <a
                                    aria-label="Solar Oven Activity mixed 7-10 Years Old Start date 12 Jan, 2024 02:00 pm and End date 12 Jan, 2024 04:00 pm"
                                    href="/en/activity/57510"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        Solar Oven
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>7-10 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Empower Young Minds with Solar Innovation &nbsp; ? Explore,
                                    Create, Cook! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            12 Jan, 2024 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            12 Jan, 2024
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231231_084923_067.png"
                                alt="BIO Matters image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Technology Zone">
                                <a
                                    aria-label="BIO Matters Activity mixed 19-24 Years Old Start date 06 Jan, 2024 02:00 pm and End date 06 Jan, 2024 04:00 pm"
                                    href="/en/activity/57502"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        BIO Matters
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>19-24 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash Creativity with Sustainable Design! &nbsp; ?
                                    Experiment, Innovate, Transform! &nbsp; Join...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            06 Jan, 2024 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            06 Jan, 2024
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231225_124703_465_0.png"
                                alt="Recipe with AI image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Software Zone">
                                <a
                                    aria-label="Recipe with AI Activity mixed 15-18 Years Old Start date 05 Jan, 2024 02:00 pm and End date 05 Jan, 2024 04:00 pm"
                                    href="/en/activity/57501"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        Recipe with AI
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>15-18 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash the Future of Recipes Innovation! &nbsp; ? Code,
                                    Create, Cook! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            05 Jan, 2024 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            05 Jan, 2024
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231210_123109_761_0.png"
                                alt="BIO Matters image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Technology Zone">
                                <a
                                    aria-label="BIO Matters Activity mixed 19-24 Years Old Start date 23 Dec, 2023 02:00 pm and End date 23 Dec, 2023 04:00 pm"
                                    href="/en/activity/57320"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        BIO Matters
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>19-24 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash Creativity with Sustainable Design! &nbsp; ?
                                    Experiment, Innovate, Transform! &nbsp; Join...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            23 Dec, 2023 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            23 Dec, 2023
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231210_123109_501_0.png"
                                alt="Recipe with AI image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Software Zone">
                                <a
                                    aria-label="Recipe with AI Activity mixed 15-18 Years Old Start date 22 Dec, 2023 02:00 pm and End date 22 Dec, 2023 04:00 pm"
                                    href="/en/activity/57319"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        Recipe with AI
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>15-18 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash the Future of Recipes Innovation! &nbsp; ? Code,
                                    Create, Cook! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            22 Dec, 2023 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            22 Dec, 2023
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                    <li className="newactivities test3">
                        <div className="figHolderNA">
                            <img
                                src="http://studio5drupaldev.applab.qa/sites/default/files/2023-12/Image_20231210_123109_892_1.png"
                                alt="3D Cakes Molds image"
                            />
                        </div>
                        <div className="textHolderNA">
                            <div className="topTextHolderNA Investigation Zone">
                                <a
                                    aria-label="3D Cakes Molds Activity mixed 11-14 Years Old Start date 16 Dec, 2023 02:00 pm and End date 16 Dec, 2023 04:00 pm"
                                    href="/en/activity/57318"
                                >
                                    <h4>
                                        <i className="topHeadNA" />
                                        3D Cakes Molds
                                    </h4>
                                </a>
                                <div className="topTextInsideNA">
                                    <span className="blackText">
                                        <i className="topBoyIconNA" /> Mixed
                                    </span>
                                    <span>11-14 Years Old</span>
                                </div>
                            </div>
                            <div className="bottomTextHolderNA">
                                <div className="bottomTextLeftNA">
                                    Unleash Your Creativity in 3D Confectionery! &nbsp; ? Model,
                                    Mold, Decorate! &nbsp; Join us at The...
                                </div>
                                <div className="bottomTextRightNA">
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> Start date{" "}
                                        <span className="date">
                                            16 Dec, 2023 <br />
                                            <small>02:00 pm</small>
                                        </span>
                                    </span>
                                    <span className="calendarNA">
                                        <i className="calendar-icons" /> End date{" "}
                                        <span className="date">
                                            16 Dec, 2023
                                            <br />
                                            <small>04:00 pm</small>
                                        </span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="btnHolderNA">
                            <button className="btn blueColor btn-disabled">Closed</button>
                        </div>
                    </li>
                </ul>
                <nav className="pagination-wrapper" aria-label="pagination">
                    <ul className="pagination">
                        <li className="active">
                            <a
                                className="undefined"
                                href="#"
                                aria-label="Go to page number 1"
                            >
                                1
                            </a>
                        </li>
                        <li className="">
                            <a className="" href="#" aria-label="Go to page number 2">
                                2
                            </a>
                        </li>
                        <li className="">
                            <a className="" href="#" aria-label="Go to page number 3">
                                3
                            </a>
                        </li>
                        <li className="">
                            <a className="" href="#" aria-label="Go to page number 4">
                                4
                            </a>
                        </li>
                        <li className="">
                            <a className="" href="#" aria-label="Go to page number 5">
                                5
                            </a>
                        </li>
                        <li className="">
                            <a
                                className="linkClassNext"
                                href="#"
                                aria-label="Go to next page"
                            >
                                Next &gt;
                            </a>
                        </li>
                    </ul>
                </nav>
            </div>




            <div className="sparkles">
                <span className="orange-circle" />
                <span className="multi-square two">
                    <b>
                        <i />
                    </b>
                </span>
                <span className="grey-square-rotate red-sq one" />
                <span className="grey-square-rotate red-sq two" />
            </div>
        </div>


        <div className="top-line ng-scope">
            <img
                src="data:image/png;base64,"
                alt=""
            />
        </div>
    </div>
</div>
