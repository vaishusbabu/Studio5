...
function TestVideo() {
    ...
    useEffect(() => {
        fetch(`https://studio5drupaldev.applab.qa/api/media-details/3?_format=json`)
            .then((res) => res.json())
            .then((data) => {
                setVideo(data);
                                ...
                                <ul>
                                    ...
                                    <li className="breadcrumb-item">
                                        <a tabIndex={0} aria-current="page" href="/en/news/3">
                                            <span>Build a VR Environment</span>
                                        </a>
                                    </li>
                                </ul>
                            </nav>
    ...
    '
                                    ...
                                    <div className="date-wrap" dangerouslySetInnerHTML={{ __html: video?.[0]?.date }} >
                                    </div>
                                    <div className="content" dangerouslySetInnerHTML={{ __html: video?.[0]?.description }}>
                        ...
                        <div className="container">
                            <div className="view-more video">
                                <Link to="/media_center">
                                    <i className="material-icons ar">arrow_backward</i> Back
                                </Link>
                            </div>
                        </div>
                    </div>
